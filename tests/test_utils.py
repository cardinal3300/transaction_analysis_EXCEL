...
def test_filter_by_date_range_month(sample_df: pd.DataFrame) -> None:
...
def test_filter_by_date_range_year(sample_df: pd.DataFrame) -> None:
    date = datetime(2024, 7, 1)
    filtered = filter_transactions_by_period(sample_df, date, "Y")
    assert len(filtered) == 5  # Все попадают в 2024 год


def test_filter_by_date_range_all(sample_df: pd.DataFrame) -> None:
...
def test_get_stock_prices_success(mock_download: MagicMock) -> None:
...
@patch("src.utils.yf.download")
def test_get_stock_prices_failure(mock_download: MagicMock) -> None:
